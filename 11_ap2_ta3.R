...
library(modelsummary);library(marginaleffects); library(mice);library(broom)
...
p1 <- with(data4,plm(policy ~ rate_v1,
                  data = data.frame(mget(ls())),index = c("MID","Wave"),
                  effects = "individual", model = "within")) 

p2 <- with(data4, plm(policy ~ rate_v1+rate_v1:rate_v2,
                      data=data.frame(mget(ls())), index = c("MID","Wave"),
                                 model = "within", effects="individual"))

p3 <- with(data4, plm(policy ~ rate_v1+rate_v1:rate_v2+rate_p,
